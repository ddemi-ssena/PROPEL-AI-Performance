...
- `views/admin/AdminDashboard.vue`:
  - Uçuş Riski: employees listesinden `risk_level=High/Medium` filtre, top 4
  - Departman yöneticisi: `user.role === 'department_manager'` ile tespit → gerçek isim
  - Departman skoru: `latest_ms` ortalaması
- `views/admin/DataManagement.vue`:
  - "Şablon İndir" butonu `adminUploadApi.downloadTemplate(dept)` çağrısına bağlandı
  - Hardcoded "Pazarlama uyarısı" → yükleme başarısında yeşil banner ile değiştirildi
- `services/api/employee.api.ts` — `getEmployee(id)` metodu eklendi
- `services/api/admin_upload.api.ts` — `downloadTemplate(dept)` metodu eklendi

**Risk Seviyesi Kaynağı** (önemli not):
- `risk_level` Excel dosyalarından **gelmez**; nabız anketinden hesaplanır
- `SurveyResponse.ars_score` (0-1): `≥ 0.6` → High, `≥ 0.2` → Medium, `< 0.2` → Low
- Seed data `seed_data.py` deterministik formülle üretir; gerçek kullanımda ML metin analizi
