...
from app.analytics.kpi_registry import KPIDefinition, get_sales_kpi_by_feature_name


SALES_TARGET_LABELS = {
    "Performance_Drop_Target": "performans dususu",
    "Burnout_Target": "tukenmislik",
    "Resignation_Target": "ayrilma riski",
    "High_Risk_Target": "yuksek risk",
}

SALES_BAND_RISK_LEVEL = {
    "Yuksek": "high",
    "Orta": "medium",
    "Dusuk": "low",
    "Evet": "high",
...
class SalesExplanationBuilder:
    @staticmethod
    def _risk_level(target_column: str, predicted_band: str) -> str:
        return SALES_BAND_RISK_LEVEL.get(str(predicted_band), "medium")
    ...
    def _threshold_status(definition: KPIDefinition, current_value: Any) -> str:
        value = SalesExplanationBuilder._numeric_value(current_value)
        if value is None:
    ...
    def _trend_signal(definition: KPIDefinition, trend_value: Any) -> str:
        trend = SalesExplanationBuilder._numeric_value(trend_value)
        if trend is None:
    ...
    def _driver_signal(feature_name: str, value: Any, definition: KPIDefinition) -> str:
        if feature_name.endswith("_trend_4"):
            return SalesExplanationBuilder._trend_signal(definition, value)
    ...
    def _driver_rationale(
        ...
        threshold_status = SalesExplanationBuilder._threshold_status(definition, current_value)
        trend_signal = SalesExplanationBuilder._trend_signal(definition, trend_value)
        value_text = SalesExplanationBuilder._format_value(current_value)
        target_label = SALES_TARGET_LABELS.get(target_column, "hedef")
    ...
    def build(
        ...
        for item in top_features:
            ...
            definition = get_sales_kpi_by_feature_name(feature_name)
            if not definition:
                continue
            ...
            drivers.append(
                {
                    ...
                    "value": SalesExplanationBuilder._format_value(value),
                    "current_value": SalesExplanationBuilder._format_value(current_value),
                    "trend_4": SalesExplanationBuilder._format_value(trend_value),
                    "threshold_status": threshold_status,
                    "trend_signal": trend_signal,
                    "signal": SalesExplanationBuilder._driver_signal(feature_name, value, definition),
                    "rationale": SalesExplanationBuilder._driver_rationale(
        ...
        risk_summary = SalesExplanationBuilder._summary_text(
            target_column=target_column,
            predicted_band=predicted_band,
    ...
    def _summary_text(
        ...
        risk_level = SalesExplanationBuilder._risk_level(target_column, predicted_band)
        if risk_level == "high":
